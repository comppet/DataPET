...
public class GrupoHistoricoController {
    ...
    private class ComparadorNomePetiano implements Comparator <Petiano> {
        ...
        public int compare(Petiano p1, Petiano p2) {
            ...
            s1 = Normalizer.normalize(p1.getNome(), Normalizer.Form.NFD)
                    .replaceAll("[^\\p{ASCII}]", "");
            
            s2 = Normalizer.normalize(p2.getNome(), Normalizer.Form.NFD)
                    .replaceAll("[^\\p{ASCII}]", "");
    ...
    public ArrayList<Aluno> getExPetianos() {
        /*Aluno aux;
        for(int i = 0; i < exPetianos.size(); i++){
        aux = exPetianos.get(i);
        exPetianos.add(aux);
        exPetianos.remove(i);
        }*/
        return exPetianos;
