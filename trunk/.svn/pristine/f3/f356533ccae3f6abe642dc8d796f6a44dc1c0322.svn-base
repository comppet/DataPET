...
public class GrupoController {
    ...
    private int passo; // o formulario de inserção é dividido em 3 passos
    private String id; // id para carregar um grupo nas operações de editar e visualizar
    private Grupo[] gruposSelecionados; // guarda os grupos que foram selecionados no datatable
    ...
    dao = new JDBCGrupoDao();
    cdao = new JDBCCursoDao();
    grupo = new Grupo();
    curso = new Curso();
    tutor = new Tutor();
    passo = 1;
        ...
        try {
            ...
            if (ex.getErrorCode() == 1062)
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
                        "O endereço de e-mail do tutor informado já existe no sistema.",
                        null));
            else
                context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
    ...
    public void validarGrupo(FacesContext context, UIComponent component, Object value) throws SQLException {
        ...
        if (grupo.getId() == 0) {
            if (grupoDao.recuperaPorSigla(sigla, LoginController.getInstituicao().getId()) == null) {
                conexao.close();
                return; // se não houver grupo cadastrado com a mesma sigla, passa pela validação com sucesso
            } else {
